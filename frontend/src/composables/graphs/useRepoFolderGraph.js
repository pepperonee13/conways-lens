  ...
  function draw({ dims, data }) {
    ...
    const folderEls = root.append('g')
      ...
      .on('mouseenter', (e, d) => {
        ...
        onShowNodeTooltip(
          {
            ...d, type: 'folder', pct: toPct(d.commits, repoTotal),
            folderFullPath: d.fullPath ?? d.id,
            folderLastCommit: d.hasChildren ? null : (d.lastCommit ?? null),
            action: d.hasChildren ? 'Click to explore subfolders' : '',
          },
